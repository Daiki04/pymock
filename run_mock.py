...
def prepare_data(file_path):
    ...
    Datapoint.data_name = file_path.name.split(".")[0]
...
def run_mock(**cl_args):
    ...
    data_file_paths = [i for i in data_file_paths if i.name != ".ipynb_checkpoints"]
        ...
        for delta_val, L in product(delta_vals, config["L"]):
            ...
            for strategy, L_comp in product(
                ...
                for run_num, run_result in enumerate(results):
                    ...
                    # print(results_dict["dataset"])
                    # Add the new results
                    results_df = results_df.append(
                        pd.DataFrame.from_dict(results_dict), ignore_index=True
                    )
                    
                    df_single = pd.DataFrame.from_dict(results_dict)
                    
                    if save_results:
                        df_single.to_csv(
                            experiment_folder / f"{Datapoint.data_name}_results_{run_num}.csv",
                            index=False
                        )
